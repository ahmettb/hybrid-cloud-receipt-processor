...
def lambda_handler(event, context):
    ...
    try:
        ...
        ai_response_text = json.loads(resp.get('body').read())['content'][0]['text']
        ...
        try:
            date_str = data.get('fis_tarihi', '2000-01-01')
            date_obj = datetime.strptime(date_str.replace('/','-'), '%Y-%m-%d')
        except:
            date_obj = datetime(2000, 1, 1) 

        sql = """INSERT INTO market_fisleri 
                 (magaza_adi, fis_tarihi, fis_saati, toplam_tutar, kdv_tutari, odeme_tipi, urun_listesi) 
        ...
        cursor.execute(sql, [
            ...
            date_obj, 
            data.get('fis_saati'), 
            float(data.get('toplam_tutar',0)), 
            float(data.get('kdv_tutari',0)), 
            data.get('odeme_tipi'), 
            json.dumps(data.get('urun_listesi'))
        ])
